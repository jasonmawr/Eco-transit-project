...
exports.MODE_COEF_CO2 = exports.ALL_LABELS = exports.XANHWRAP_PRESETS = void 0;
exports.isRushHour = isRushHour;
exports.assignXanhWrapLabel = assignXanhWrapLabel;
exports.calculateXanhWrapStats = calculateXanhWrapStats;
exports.XANHWRAP_PRESETS = [
...
function assignXanhWrapLabel(legs) {
    if (!legs || legs.length === 0) {
        return exports.ALL_LABELS.find(l => l.code === 'model_employee');
    }
    ...
    if (publicRatio === 1 && totalKm >= 20) {
        return exports.ALL_LABELS.find(l => l.code === 'no_smoke_absolute');
    }
    // #2 TAY LÁI VỀ HƯU: 100% bus/metro
    if (publicRatio === 1) {
        return exports.ALL_LABELS.find(l => l.code === 'retired_driver');
    }
    // #3 RA GA NHƯ VỀ NHÀ: >= 60% bus/metro
    if (publicRatio >= 0.6) {
        return exports.ALL_LABELS.find(l => l.code === 'station_regular');
    }
    // #4 TẬP TÀNH BỎ XE: >= 1 leg bus/metro
    if (publicLegs.length >= 1) {
        return exports.ALL_LABELS.find(l => l.code === 'quitting_rookie');
    }
    // #5 TRÙM CUỐI XA LỘ: totalKm >= 35
    if (totalKm >= 35) {
        return exports.ALL_LABELS.find(l => l.code === 'highway_boss');
    }
    // #6 CÚ ĐÊM CHÍNH HIỆU: any leg depart_time >= 21:00
    const hasNightLeg = legs.some(l => {
        if (!l.depart_time || !l.depart_time.includes(':'))
            return false;
        const h = parseInt(l.depart_time.split(':')[0], 10);
        return h >= 21;
    });
    if (hasNightLeg) {
        return exports.ALL_LABELS.find(l => l.code === 'night_owl');
    }
    // #7 TEAM 5 GIỜ SÁNG: first leg depart_time < 06:30
    if (legs[0]?.depart_time) {
        const [h, m] = legs[0].depart_time.split(':').map(Number);
        if ((h * 60 + m) < (6 * 60 + 30)) {
            return exports.ALL_LABELS.find(l => l.code === 'team_5am');
        }
    }
    // #8 SPEEDRUN THÀNH PHỐ: legs.length >= 6
    if (legs.length >= 6) {
        return exports.ALL_LABELS.find(l => l.code === 'city_speedrun');
    }
    ...
    if (rushHourLegsCount >= 2) {
        return exports.ALL_LABELS.find(l => l.code === 'rush_hour_god');
    }
    // #10 KẸT XE LÀ BẠN THÂN: avg speed < 18 km/h
    const avgSpeed = totalMin > 0 ? (totalKm / (totalMin / 60)) : 25;
    if (avgSpeed < 18) {
        return exports.ALL_LABELS.find(l => l.code === 'traffic_bestie');
    }
    ...
    if (motorbikeMins >= 150) {
        return exports.ALL_LABELS.find(l => l.code === 'steel_spine');
    }
    // #12 XE ÔM CỦA CHÍNH MÌNH: >= 4 legs
    if (legs.length >= 4) {
        return exports.ALL_LABELS.find(l => l.code === 'self_driver');
    }
    // #13 CON LẮC HAI ĐẦU: exactly 2 legs & totalKm >= 12
    if (legs.length === 2 && totalKm >= 12) {
        return exports.ALL_LABELS.find(l => l.code === 'two_end_pendulum');
    }
    // #14 DÂN NỘI ĐÔ XỊN: all legs < 5km
    const allShort = legs.every(l => l.distance_km < 5);
    if (allShort) {
        return exports.ALL_LABELS.find(l => l.code === 'downtown_native');
    }
    // #15 NGƯỜI CỦA GIỜ VÀNG: no legs in rush hour
    const noRushHour = legs.every(l => !isRushHour(l.depart_time));
    if (noRushHour) {
        return exports.ALL_LABELS.find(l => l.code === 'golden_hour');
    }
    // #16 Fallback: NHÂN VIÊN KIỂU MẪU
    return exports.ALL_LABELS.find(l => l.code === 'model_employee');
}
function calculateXanhWrapStats(legs) {
    ...
    let co2SavedGrams = 0;
    legs.forEach(l => {
        const currentCoef = exports.MODE_COEF_CO2[l.mode] ?? 56;
...
//# sourceMappingURL=xanhwrapCore.js.map